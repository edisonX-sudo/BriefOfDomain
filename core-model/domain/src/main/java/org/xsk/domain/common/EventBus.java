...
public class EventBus {
    ...
    final static ThreadLocal<Queue<DomainEvent>> BEFORE_EVENT_EMITTED_EVENT_WAITING_QUEUE = ThreadLocal.withInitial(ArrayDeque::new);
    final static ThreadLocal<Queue<DomainEvent>> AFTER_EVENT_EMITTED_EVENT_WAITING_QUEUE = ThreadLocal.withInitial(ArrayDeque::new);
    ...
    public static <E extends DomainEvent> void fire(E event) {
        ...
        BEFORE_EVENT_EMITTED_EVENT_WAITING_QUEUE.get().add(event);
        AFTER_EVENT_EMITTED_EVENT_WAITING_QUEUE.get().add(event);
    }

    public static void triggerBeforeEventEmitted() {
        triggerEventEmitted(BEFORE_EVENT_EMITTED_EVENT_WAITING_QUEUE, DomainPolicy.SubscribePoint.BEFORE_EVENT_EMITTED);
    }

    public static void triggerAfterEventEmitted() {
        triggerEventEmitted(AFTER_EVENT_EMITTED_EVENT_WAITING_QUEUE, DomainPolicy.SubscribePoint.AFTER_EVENT_EMITTED);
    }
