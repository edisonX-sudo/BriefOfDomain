...
### con复杂度角度看待3层架构
* 3层架构分controller/service/dao,从复杂度的角度他分离了控制器框架的复杂度和数据库的复杂度
(这两者是web开发的常见复杂度部分),然而service的复杂度里面仍然包含其他技术复杂度和领域复杂度,故
3层架构一定程度分隔复杂度,但面对复杂领域时这种架构仍会使得开发者在理解和处理复杂度时力不从心
